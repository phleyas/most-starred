import { signalStore, withState, withHooks, withMethods, patchState, withLinkedState } from '@ngrx/signals';
...
export const DashboardStore = signalStore(
  ...
  withTreeShakableDevTools('Dashboard Store'),
  ...
  withMethods(
    (store, locationsDataService = inject(LocationsDataService), sensorsDataService = inject(SensorsDataService)) => ({
      ...
      async loadSensors(selectedLocationId: number): Promise<void> {
        patchState(store, { isLoadingSensors: true, sensors: [] });
        try {
      ...
      async loadLocations(): Promise<void> {
        patchState(store, { isLoadingLocations: true, locations: [] });
        try {
